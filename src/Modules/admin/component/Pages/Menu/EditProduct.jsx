...
const EditProduct = ({ open, setOpen, editingCategory,setCount }) => {
    ...
    useEffect(() => {
        const fetchCategoryData = async () => {
            if (editingCategory) {
                try {
                    // Assuming editingCategory has the necessary data
                    setNewProduct({
                        name: editingCategory.name,
                        price: editingCategory.price,
                        description: editingCategory.description,
                        discount: editingCategory.discount,
                    });
                    console.log(editingCategory,'editingCategory');

                    // Assuming category is an object with _id
                    setProductCategory({
                        id: editingCategory?.category?._id,
                        name: editingCategory?.category?.name,
                    });
                    // Set the default image if available
                    setImage(editingCategory?.image || null);
 ...
 const handleSave = async () => {
    ...
    try {
        ...
        formData.append('category', productCategory?.id);
        if (image) {
            formData.append('image', image);
        }
        const response = await axios.put(`${host}/api/admin/updateproduct/${editingCategory?._id}`, formData);
                ...
                >
                    <Fade in={open}>
                        <Box sx={style}>
                            ...
                            />
                            <Typography>
                                Current Image: {image?.name||image || 'No image uploaded yet'}
                            </Typography>
                            <TextField
                                label="Description"
                                name="description"
